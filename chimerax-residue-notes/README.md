...
- open a `Nearby Notes` tool window for read-only nearby-note browsing around CoFR
...
- optional short titles per note, stored in the custom mmCIF loop
...
- `title`, `author`, and `note` are Base64-encoded UTF-8 text for robust round-tripping
...
For the read-only nearby-notes pane:
